...
<html lang="en">
  ...
  <body>
    <div class="parent">
      ...
      <div class="bmp">
        ...
        <div class="content">
          <div class="temp">Temperature : <span id="temp">0</span> °C</div>
          <div class="press">Pressure : <span id="press">0</span> hPa</div>
          <div class="bmp-alt">
        ...
        <div class="footer">
          <button
            class="btn"
            id="bmp-log"
            onclick="DownloadModuleLog('BMP280')"
          >
            📤 Download BMP280 Log
          </button>
        </div>
      ...
      <div class="gyro">
        ...
        <div class="content-lr">
          <div class="content-left">
            <div class="ax">X : <span id="ax">0</span> g</div>
            <div class="ay">Y : <span id="ay">0</span> g</div>
            <div class="az">Z : <span id="az">0</span> g</div>
          </div>
          <div class="content-right">
            <div class="gx">X : <span id="gx">0</span> °/s</div>
            <div class="gy">Y : <span id="gy">0</span> °/s</div>
            <div class="gz">Z : <span id="gz">0</span> °/s</div>
          </div>
        </div>

        <div class="footer">
          <button class="btn" id="gy-log" onclick="DownloadModuleLog('GY-521')">
            📤 Download GY-521 Log
          </button>
        </div>
      </div>
      ...
      <div class="gps">
        ...
        <div class="footer">
          <button class="btn" id="gps-log" onclick="DownloadModuleLog('GPS')">
            📤 Download GPS Log
          </button>
        </div>
      </div>

      <div class="plot-bmp">14</div>
      <div class="plot-gy">15</div>

      <div class="foot">
        <div class="foot-left">
          <div class="title">Status</div>
          <div class="launch">Launch : <span id="launch">🔴</span></div>

          <div class="flight">In Flight : <span id="flight">🔴</span></div>

          <div class="deploy">Deployment : <span id="deploy">🔴</span></div>

          <div class="land">Landed : <span id="land">🔴</span></div>
        </div>
        ...
        <div class="foot-right">
          <button class="btn" id="log" onclick="Download()">
            📤 Download Log
          </button>
        </div>
      </div>
  ...
  <script>
    ...
    function initMap(lat, lon) {
      ...
      defaultMarker
        .bindPopup(
          `ตำแหน่งที่ ${count_marker} | เวลา : ${run} sec<br>Latitude: ${lat}<br>Longitude: ${lon}`
        )
        .openPopup();
    }
    ...
    function updateMarker(lat, lon) {
      ...
      mark
        .bindPopup(
          `ตำแหน่งที่ ${count_marker} | เวลา : ${run} <br>Latitude: ${lat}<br>Longitude: ${lon}`
        )
        .openPopup();
    }
    ...
    function Clock() {
      setInterval(() => {
        ...
        const formatted =
          `UTC+7 ${pad(utc7.getUTCDate())}-${
            months[utc7.getUTCMonth()]
          }-${String(utc7.getUTCFullYear()).slice(2)} ` +
          `${pad(utc7.getUTCHours())}:${pad(utc7.getUTCMinutes())}:${pad(
            utc7.getUTCSeconds()
          )}`;
    ...
    function dataStreamTester() {
      for (let i = 0; i < 100; i++) {
        ...
        switch (type) {
          ...
          case 3:
            // Structure: [1, [1, int]]
            dataStream.push([1, [Math.floor(Math.random() * 6) + 1]]);
            break;
        }
      }
    }
